...
if (lAutoDetectGenesis) and (FileSz > (5820{114+35+4})) then
   ...
   if (ord(tx[0])=206) and (ord(tx[1])=250) then
      ...
      if ((Group=160) or(Group =256) or (Group= 340) or (Group=512) or (group =640)) and
           ((element=160) or (element =256) or (element= 340) or (element=512) ) then
         begin
         CloseFile(fp);
         if lDiskCacheSz > 0 then
            freemem(lDiskCacheRA);
   ...
   lGenesis := false;
   if ((tx[0] <> 'I') OR (tx[1] <> 'M') OR (tx[2] <> 'G') OR (tx[3] <> 'F')) then
      begin {DAT format}
...
if (lAutoDetectInterfile) and (FileSz > 256) and (not lDICM_at_128) then
   ...
   if TmpStr = '.VOL' then
      ...
      if (tx[0] = 'm') and (tx[1] = 'd') and (tx[2] = 'v') and (tx[3] = 'o')
         and (tx[4] = 'l') and (tx[5] = '1') then
         begin
         lDicomData.ImageStart := read32(fp,lrOK);//1078;
         lDICOMdata.little_endian := 1;
         lDicomData.XYZdim[1] := read32(fp,lrOK);
   ...
   if (TmpStr = '.PGM') or (TmpStr = '.PPM') then
      ...
      dseek(fp, 0);
      lWord   := read16(fp,lrOK);
      if (lWord=13648){'P5'=1x8BIT GRAYSCALE} or (lWord=13904) {'P6'=3x8bit RGB} then
         begin //bitmap signature
         {repeat
         ...
         if (lWord=12880){'P2'=1x8BIT ASCII} or (lWord=13136) {'P3'=3x8bit ASCI} then
            begin
   ...
   liPos := 1;
   lStr :='';
   While (liPos <= 20) and (lStr <> 'INTERFILE') do
      begin
      if tx[liPos] in ['i','n','t','e','r', 'f'{,'i'},'l'{,'e'},'I','N','T','E',
         'R', 'F'{,'I'},'L'{,'E'}] then
...
if (tx[0] = 'S') and (tx[1] = 'I') and (tx[2] = 'E') and (tx[3] = 'M') and (tx[4] = 'E')
   and (tx[5] = 'N') and (tx[6] = 'S') then
   begin
   CloseFile(fp);
   if lDiskCacheSz > 0 then
      freemem(lDiskCacheRA);
...
dseek(fp,0);
dBlockRead(fp, tx, 8*SizeOf(Char), n);
if (tx[0]='V') and (tx[1]='-') and (tx[2]='d') and (tx[3] = 'a')and (tx[4]='t') and (tx[5] = 'a') then
   begin
   CloseFile(fp);
   if lDiskCacheSz > 0 then
      freemem(lDiskCacheRA);
...
if (lAutoDECAT7) and (tx[0]='M') and (tx[1]='A') and (tx[2]='T') and (tx[3]='R') and
   (tx[4]='I') and (tx[5]='X') then
   begin
   CloseFile(fp);
   if lDiskCacheSz > 0 then
      freemem(lDiskCacheRA);
...
if (ord(tx[0]) = 1) and (ord(tx[1])=2) and ((ljunk mod lMatrixSz)=0){128*128*2bytes = 32768} then
   begin //Picker PRISM
   lDicomData.little_endian := 0;
...
777:
   ...
   if explicitVR or first_one then
      ...
      if group = $FFFE then else //1384  - ACUSON images switch off ExplicitVR for file image fragments
         if  ((lT0=kO) and (lT1=kB)) or ((lT0=kU) and (lT1=kN)){<-UN added} or ((lT0=kO)
            and (lT1=kW)) or ((lT0=kS) and (lT1=kQ)) then
            begin
            ...
            if ((lT3=kO) and (lT2=kB)) or ((lT3=kU) and (lT2=kN)){<-UN added} or((lT3=kO)
               and (lT2=kW)) or ((lT3=kS) and (lT2=kQ)) then
               begin
               e_len:= read32(fp,lrOK);
               if not lrOK then goto 666;
               if first_one then explicitVR := true;
               ...
               if  ( ((lT0=kA) and (lT1=kE)) or ((lT0=kA) and (lT1=kS))
                  ...
                  or ((lT0=kS) and (lT1=kS)) or ((lT0=kS) and (lT1=kT)) or ((lT0=kT) and (lT1=kM))
                  or ((lT0=kU) and (lT1=kI)) or ((lT0=kU) and (lT1=kL)) or ((lT0=kU) and (lT1=kS))
                  or ((lT0=kA) and (lT1=kE)) or ((lT0=kA) and (lT1=kS)) ) then
                  begin
                  lGrpStr := chr(lT0) + chr(lT1);
                  if lDicomData.little_endian = 1 then
                  ...
                  if (((lT3=kA) and (lT2=kT)) or ((lT3=kC) and (lT2=kS)) or ((lT3=kD)
                     and (lT2=kA)) or ((lT3=kD) and (lT2=kS))
                     ...
                     or ((lT3=kU) and (lT2=kI)) or ((lT3=kU) and (lT2=kL)) or ((lT3=kU) and (lT2=kS))) then
                        begin
                        if lDicomData.little_endian = 1 then
                           e_len := (256 * lT0) + lT1
                          else
   ...
   if (first_one) and (lDicomdata.little_endian =0) and (e_len = $04000000) then
      begin
      ...
      if (first_one) and (lDicomData.little_endian =1) and (e_len = $04000000) then
         begin
         ShowMessage('Switching to little endian');
         lDicomData.little_endian := 0;
         dseek(fp, where);
   ...
   case group of
      ...
      $0008 : case element of
         ...
         $08 :  begin
            ...
            dBlockRead(fp, buff{^}, e_len, n);
            i := e_len -1;
            while (i>-1) and (Char(buff[i]) in ['a'..'z','A'..'Z',' ']) do
               begin
         ...
         $100: info := 'Code Value';
         $102 : begin info := 'Coding Schema Designator'; t := _string; end;
      ...
      $0009: if element = $0010 then
         ...
         if e_len > 4 then
            ...
            FreeMem( buff);
            remaining := 0;
            if (Length(TmpStr)>4) and (TmpStr[1]='M') and (TmpStr[2]='E') and
               (TmpStr[3]='R') and (TmpStr[4]='G') then
               lOldSiemens_IncorrectMosaicMM := true; //detect MERGE technologies mosaics
            e_len := 0; {use tempstr}
            //if (TmpStr[1] = 'M') then
            end;
         end;
      $0010 : case element of
...
{            xx           DICOMHeaderString(TmpStr);
 ...
 magma
         ...
         $1050 :  begin t := _string; info := 'Spatial Resolution'; end;
         ...
         $1149: begin t := _string; info := 'Field of View Dimension[s]'; end;
         $1150: begin info := 'Exposure Time [ms]'; t := _string; end;
         $1151: begin
         ...
         $1190 : begin info := 'Focal Spot[s]'; t := _string; end;
         $11A0 : begin info := 'Body Part Thickness'; t := _string; end;
         ...
         $1260 :  begin t := _string; info := 'Plate Type'; end;
         $1261 :  begin t := _string; info := 'Phosphor Type';  end;
         ...
         $1312: begin t := _string; info := 'Phase Encoding Direction'; end;
         $1314: begin t := _string; info := 'Flip Angle'; end;
         $1315: begin t := _string;info := 'Variable Flip Angle Flag'; end;
         $1316: begin t := _string;info := 'SAR'; end;
	 ...
	 $00 :  info := 'Relationship Group Length';
      ...
      $0021 : case element of
         ...
         $134F: begin //1366
            ...
            dBlockRead(fp, buff{^}, e_len, n);
            for i := 0 to e_len-1 do
               if Char(buff[i]) in ['?','A'..'Z','a'..'z'] then
                  lStr := lStr +upcase(Char(buff[i]));
            FreeMem( buff);
         ...
         case element of
            ...
            $04 :  begin
               ...
               if TmpStr = 'MONOCHROME1' then
                  lDicomdata.monochrome := 1
                 else
                  if TmpStr = 'MONOCHROME2' then
                     lDicomdata.monochrome := 2
                    else
                     if (length(TMpStr)> 0) and (TmpStr[1] = 'Y') then
                        lDICOMdata.monochrome := 4
                       else
                        lDICOMdata.monochrome := 3;
               remaining := 0;
               e_len := 0; {use tempstr}
               end;
            ...
            $100: begin
               ...
               if tmp = 8 then
                  ...
                  if tmp = 12 then
                     lDicomData.Allocbits_per_pixel := 12
                    else
                     if tmp = 16 then
                        lDicomData.Allocbits_per_pixel := 16
                       else
                           ...
                           if tmp = 32 then
                              ...
                              if lWord in [8,12,16,24,32] then
                                 ...
                                 if lImageFormatOK then
                                    Showmessage('This software only reads 8, 12, 16, 24 and 32 bit DICOM files. This file allocates '+inttostr(tmp)+' bits per voxel.');
                                 lImageFormatOK := false;
                                 end;
                              end;
                      ...
                      if tmp <= 24 then
                         ...
                         if tmp <= 32 then
                            ...
                            if lWord in [8,12,16,32] then
                               ...
                               begin
                               if lImageFormatOK then
                                  Showmessage('This software can only read 8, 12, 16 and 32 bit DICOM files. This file stores '+inttostr(tmp)+' bits per voxel.');
                               lDicomData.Storedbits_per_pixel := tmp;
                               lImageFormatOK := false;
                               end;
                            end;
            ...
            $1201, $1202,$1203: begin
               ...
               if dFilePos(fp) > (filesz-remaining) then goto 666;
      ...
      $2010 : case element of
         $0: info := 'Film Box Group Length';
         $100: info := 'Border Density';
         end;

      {DICOM RT extensions added by ACC 13/6/2011}
      $3002 : case element of
         $0002 : info := 'RT Image Label';
         $0003 : info := 'RT Image Name';
         $0004 : info := 'RT Image Description';
         $000A : info := 'Reported Values Origin';
         $000C : info := 'RT Image Plane';
         $000D : info := 'X-Ray Image Receptor Translation';
         $000E : info := 'X-Ray Image Receptor Angle';
         $0010 : info := 'RT Image Orientation';
         $0011 : begin
            {This overides 0028:0030 Pixel Spacing}
            info := 'Image Plane Pixel Spacing';
            readfloats (fp, remaining, TmpStr, lfloat1, lfloat2, lROK);
            if not lrOK then goto 666;
            //row spacing [y], then column spacing [x]: see part 3 of DICOM
            e_len := 0;      remaining := 0;
         ...
         $0012 : info := 'RT Image Position';
         $0020 : info := 'Radiation Machine Name';
         $0022 : info := 'Radiation Machine SAD';
         $0024 : info := 'Radiation Machine SSD';
         $0026 : info := 'RT Image SID';
         $0028 : info := 'Source to Reference Object Distance';
         $0029 : info := 'Fraction Number';
         $0030 : info := 'Exposure Sequence';
         $0032 : info := 'Meterset Exposure';
         $0034 : info := 'Diaphragm Position';
         $0040 : info := 'Fluence Map Sequence';
         $0041 : info := 'Fluence Data Source';
         $0042 : info := 'Fluence Data Scale';
         end; {of group 3002}
         ...
         case element of
            $E000 : begin
               ...
               if not lProprietaryImageThumbnail then
                  begin
                  if (lReadJPEGtables) and ((lDICOMdata.RunLengthEncoding) or
                     (lDICOMdata.JPEGLossyCpt) or (lDICOMdata.JPEGLosslessCpt)) and
                     (not lFirstFragment) and (e_len > 1024) {1384} and ( (e_len+dFilePos(fp)) <= FileSz) then
                     begin
   ...
   //                   showmessage('abba'+inttostr(lJPEGEntries)+'xxx'+inttostr(dFilePos(fp)));
                  ...
                  if (lDICOMdata.CompressOffset =0) and ( (e_len+dFilePos(fp)) <= FileSz) and
                     (e_len > 1024){ALOKA} then
                        begin
                  ...
                  //showmessage(inttostr(e_len));
                  if (e_len > 1024) and (lDICOMdata.CompressSz=0) then
                     begin //ABBA RLE ALOKA
                     //Time_To_Quit := true;//ABBA
                     lDICOMdata.CompressSz := e_len;
                     lDICOMdata.CompressOffset := dFilePos(fp);
                     end;
                  if (lFirstFragment) or ((e_len > lDICOMdata.CompressSz) and not
                     (lDicomData.RunLengthEncoding)) then
                     lDICOMdata.CompressOffset := dFilePos(fp);
            ...
            $E0DD : begin
               ...
               if (lDICOMdata.XYZdim[1]<lDICOMdataBackUp.XYZdim[1]) then
                  begin
                  ...
                  if (lDicomData.RunLengthEncoding) or ( ((lDicomData.JPEGLossycpt) or
                     (lDicomData.JPEGLosslesscpt)) and
                     (gECATJPEG_table_entries >= lDICOMdata.XYZdim[3])) then
                     time_to_quit :=  TRUE;
                //RLE ABBA
               if (e_len = 0) and (lDICOMdata.CompressSz = 0) and
                  (lDicomData.RunLengthEncoding) then
                  begin //ALOKA
                  explicitVR := true;
      ...
      $7FE0 : case element of
         ...
         $10 :  begin
            ...
            if (lDICOMdata.XYZdim[1]<lDICOMdataBackUp.XYZdim[1]) then
               ...
               if (not lDicomData.RunLengthEncoding) and (not lDicomData.JPEGLossycpt) and
                  (not lDicomData.JPEGLosslesscpt) then
                  begin
                  time_to_quit := TRUE;
   ...
   if (e_len + dfilepos(fp)) > FileSz then
      begin//patch for GE files that only fill top 16-bytes w Random data
      e_len := e_len and $FFFF;
      end;
   if (e_len > {x$FFFF}131072) {and (dfilepos(fp) > FileSz)} then
      begin
   ...
   if (NOT time_to_quit) AND (e_len > 0) and (remaining > 0) then
      ...
      if (e_len + dfilepos(fp)) > FileSz then
         begin
         if (lDICOMdata.GenesisCpt) or (lDICOMdata.JPEGlosslessCpt) or
            (lDICOMdata.JPEGlossyCpt) then
            lHdrOK := true
           else
            showmessage('Error: Dicom header exceeds file size.'+kCR+lFilename);
         goto 666;
         end;
      ...
      if e_len > 0 then
         ...
         if lVerboseRead then
            case t of
               ...
               i8, i16, i32, ui8, ui16, ui32, _string  : for i := 0 to e_len-1 do
                  if Char(buff[i]) in ['+','-','/','\',' ', '0'..'9','a'..'z','A'..'Z'] then
                     lStr := lStr +(Char(buff[i]))
                    else
                     lStr := lStr +('.');
   ...
   if (lGrp{info = 'identifying group'})  then
      if MessageDlg(lStr+'= '+info+' '+IntToHex(where,4)+': ('+IntToHex(group,4)+','
         +IntToHex(element,4)+')' + IntToStr(e_len) + '. Continue?',
   ...
   if lverboseRead then
      ...
      if length(lDynStr) > kMaxTextBuf then
         ...
         lDynStr := lDynStr {+chr(lT0)+chr(lT1)+' '+inttostr(dfilepos(fp))+'abba'}+
            IntToHex(group,4)+','+IntToHex(element,4)+','{+inttostr(where)+': '+lGrpStr}
            +Info+'='+lStr+kCR ;
...
if (lMatrixSz > 1) and ((lDicomdata.XYZdim[1] mod lMatrixSz) = 0) and
   ((lDicomdata.XYZdim[2] mod lMatrixSz) = 0) then
   begin
   ...
   if {not} lOldSiemens_IncorrectMosaicMM then
      begin //old formats convert size in mm incorrectly - modern versions are correct and include transfer syntax
      //showmessage('abba'+inttostr(lMatrixSz));
   ...
   if (lSiemensMosaic0008_0008) and (lPhaseEncodingSteps > 0) and
      (lPhaseEncodingSteps < lDicomdata.XYZdim[2]) and ((lDicomdata.XYZdim[2]
      mod lPhaseEncodingSteps) = 0) and ((lDicomdata.XYZdim[2] mod (lDicomdata.XYZdim[2]
      div lPhaseEncodingSteps)) = 0) then
      begin
